...
from .raw import EfficientAdModel, EfficientAdModelSize, reduce_tensor_elems
...
class EfficientAd(AnomalyModule):
    ...
    def prepare_imagenette_data(self) -> None:
        ...
        imagenet_dataset = ImageFolder(
            imagenet_dir, transform=TransformsWrapper(t=self.data_transforms_imagenet)
        )
        self.imagenet_loader = DataLoader(
            imagenet_dataset, batch_size=self.batch_size, shuffle=True, pin_memory=True
        )
    ...
    def teacher_channel_mean_std(self, dataloader: DataLoader) -> dict[str, Tensor]:
        ...
        for batch in tqdm.tqdm(
            dataloader, desc="Calculate teacher channel mean", position=0, leave=True
        ):
        ...
        for y in tqdm.tqdm(
            teacher_outputs,
            desc="Calculate teacher channel std",
            position=0,
            leave=True,
        ):
        ...
        channel_var = torch.mean(torch.stack(means_distance), dim=0)[
            None, :, None, None
        ]
    ...
    def training_step(
        ...
        loss_st, loss_ae, loss_stae = self.model(
            batch=batch["image"], batch_imagenet=batch_imagenet
        )
        ...
        self.log(
            "train_stae", loss_stae.item(), on_epoch=True, prog_bar=True, logger=True
        )

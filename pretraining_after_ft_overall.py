...
class MyDataset(Dataset):
    ...
    def transform_image(self, img_path):
        ...
        return train_transform(image,self.image_size)
...
def main(args):
    ...
    if args.network == "wide_resnet101_2":
        ...
        state_dict = torch.load(
            os.path.join(args.ft_folder, f"ft_vit.pth"),
            map_location=device,
        )
        pretrained_weights = {}
    ...
    train_set = MyDataset(args.mvtec_loco_path, args.extractor_input_size)
        ...
        if iteration % 10000 == 0:
            if not on_gpu or dist.get_rank() == 0:
                ...
                torch.save(
                    ...
                    os.path.join(
                        args.output_folder,
                        f"tmp_teacher_{args.pdn_size}_state_{suffix}_ft.pth",
                    ),
                )
    if not on_gpu or dist.get_rank() == 0:
        # torch.save(
        ...
        torch.save(
            ...
            os.path.join(
                args.output_folder,
                f"teacher_{args.pdn_size}_state_{suffix}_ft.pth",
            ),
        )

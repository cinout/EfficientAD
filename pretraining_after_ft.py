...
def main(args):
    ...
    if args.network == "wide_resnet101_2":
        ...
        state_dict = torch.load(
            os.path.join(args.ft_folder, f"ft_vit_{args.subdataset}.pth"),
            map_location=device,
        )
        pretrained_weights = {}
        ...
        # FIXME: initialize extractor [end]
    ...
    train_set = ImageFolderWithoutTarget(
        os.path.join(args.mvtec_loco_path, args.subdataset, "train"),
        transform=input_transform_func,
    )
    # FIXME: change dataset [end]
        ...
        if iteration % 10000 == 0:
            if not on_gpu or dist.get_rank() == 0:
                ...
                torch.save(
                    ...
                    os.path.join(
                        args.output_folder,
                        f"tmp_teacher_{args.pdn_size}_state_{suffix}_ft_{args.subdataset}.pth",
                    ),
                )
    if not on_gpu or dist.get_rank() == 0:
        # torch.save(
        ...
        torch.save(
            ...
            os.path.join(
                args.output_folder,
                f"teacher_{args.pdn_size}_state_{suffix}_ft_{args.subdataset}.pth",
            ),
        )

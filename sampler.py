...
class GreedyCoresetSampler(BaseSampler):
    ...
    def _compute_batchwise_differences(
        ...
        a_times_a = (
            matrix_a.unsqueeze(1).bmm(matrix_a.unsqueeze(2)).reshape(-1, 1)
        )  # shape: [N, 1]
        b_times_b = (
            matrix_b.unsqueeze(1).bmm(matrix_b.unsqueeze(2)).reshape(1, -1)
        )  # shape: [1, N]
    ...
    def _compute_greedy_coreset_indices(self, features: torch.Tensor) -> np.ndarray:
        ...
        )  # Frobenius norm of each N, shape: N
        ...
        for _ in range(num_coreset_samples):
            ...
            )  # shape: [N, 2], first value is Frobenius norm (normed distance to all points) of each N, second value is distance to the selected coreset item

...
class ChannelWiseTransformerBlock(nn.Module):
    def __init__(
        self, num_patches, patch_dim=1, dim=64, heads=5, dim_head=64, dropout=0.0
    ):
        super().__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(patch_dim)  # FIXME: diff from 3d
    ...
    def forward(self, z):
        x = self.avg_pool(z)
        x = x.flatten(-2)  # FIXME: diff from 3d

        x = self.projection(x)
        x += self.pos_embedding

        x = self.mha(x)
        x = x.mean(-1).unsqueeze(-1).unsqueeze(-1)  # FIXME: diff from 3d
        x = self.sigmoid(x)
...
class SSMCTB(nn.Module):
    def __init__(self, channels, kernel_dim=1, dilation=1):
        ...
        self.relu = nn.ReLU()
        self.transformer = ChannelWiseTransformerBlock(
            num_patches=channels, patch_dim=1
        )

        self.conv1 = nn.Conv2d(
            in_channels=channels, out_channels=channels, kernel_size=kernel_dim
        )
